...
const UiDeveloper = () => {
  ...
  const workedWith = [
   ...
   const openPersonModal = (person) => {
    ...
    return (
      <div>
         {/* UI Developer Section */}
        <div className="bg-white p-6 rounded-lg shadow-lg">
          <p className="text-sm mb-4">Inicio Profile</p>
          <h3 className="text-2xl font-bold mb-4">UI Developer</h3>
          <p className="text-gray-600 mb-4 p-2 text-[8px] bg-blue-100 rounded-lg whitespace-nowrap items-center">Lorem ipsum the best sentence world of text</p>
          <div className="mb-6">
            <div className='flex justify-between'>
                <h4 className="text-lg font-semibold">Worked with</h4>
                <p className="text-[#ED2590] mt-2 inline-block">View all</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-3 gap-2 mt-4">
              {workedWith.slice(0, 6).map((person, index) => (
                <div
                  key={index}
                ...
                >
                  ...
                  <p className="text-[11px] mt-2 text-justify whitespace-nowrap">{person.name}</p>
                </div>
              ))}
            </div>
          </div>
